 ...
 *******************************************************/
 class BKDTree{
    ...
    /**
     * To check if the two points are the same ot no.
     * @param first is the first point
     * @param seconf is the second point
     * @return Node
     */
     def same(first: Point, second:Point):Boolean={
        for(i<-0 to 1)
    ...
    def insertBKD(current:Node, value:Point, depth: Int ):Node ={
        ...
        if(current==null||same(current.point,value))
          return null
        var cd:Int=depth%2
     ...
     */
     ...
     def insertUser(value:Point):Unit={
         ...
         if(current==null)
         {
             ...
             current.rect=new Rectangle(0, 0, 10000000, 10000000);
